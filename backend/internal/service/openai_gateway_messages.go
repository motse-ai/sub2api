					...
					if err := json.Unmarshal([]byte(payload), &event); err == nil {
						...
						if isOpenAICompatResponsesTerminalEvent(event.Type) && event.Response != nil {
							...
							if event.Type == "response.failed" {
								...
								return event.Response, usage, acc,
									&UpstreamFailoverError{StatusCode: http.StatusBadGateway, ResponseBody: []byte(fmt.Sprintf(`{"error":{"type":"upstream_error","message":"upstream response failed: %s"}}`, errCode))}
							}
...
func (s *OpenAIGatewayService) handleAnthropicStreamingResponse(
	...
	// Defer header write until we confirm the first upstream event is not an error.
	writeHeaders := func() {
		if s.responseHeaderFilter != nil {
			responseheaders.WriteFilteredHeaders(c.Writer.Header(), resp.Header, s.responseHeaderFilter)
		}
	...
	processDataLine := func(payload string) bool {
		...
		if event.Type == "response.failed" {
			...
			if event.Response != nil && event.Response.Error != nil {
				failedMessage = event.Response.Error.Code
			}
		}
	...
	finalizeStream := func() (*OpenAIForwardResult, error) {
		if sawFailedEvent {
			...
			if !clientOutputStarted {
				// No data sent yet — safe to failover to another account
				return resultWithUsage(), &UpstreamFailoverError{
					StatusCode:   http.StatusBadGateway,
					ResponseBody: []byte(fmt.Sprintf(`{"error":{"type":"upstream_error","message":"%s"}}`, failedMessage)),
				}
			}
			// Content already sent to client — just error, don't switch accounts
			return resultWithUsage(), fmt.Errorf("upstream response failed mid-stream: %s", failedMessage)
		}
